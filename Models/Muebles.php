...
class Muebles{
	...
	//Métodos que puede accesar un objeto, nos dejan hacer operaciones sobre él
	   /*Dentro del controlador se mandan llamar así: 
		...
		public function llenaDatosCompletos($idMueble, $desAbreviada, $desDetallada, $ubicacion,$latitud, $longitud, $reservado, $usuario){
		...
		/* Método que sirve para llenar todos los datos de un objeto vacío
		...
		public function cargarMueble($id){
			...
			if($row = mysql_fetch_array($result)){
				$this ->llenaDatosCompletos($row['idMueble'], $row['desAbreviada'], $row['desDetallada'] ,  $row['ubicacion'], $row['latitud'], $row['longitud'], $row['reservado'], $row['usuario']);
			}
			else{
	...
	//Métodos que se puede accesar una clase sin necesidad de crear un objeto Muebles
